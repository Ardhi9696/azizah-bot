...
def akses_progress(driver, prefer_row2: bool = True) -> dict:
    ...
    purple = soup.select(SEL["tables_purple"])
    pengiriman_list = parse_pengiriman_table(purple[0]) if purple else []
    riwayat = parse_riwayat_table(purple[1]) if len(purple) >= 2 else []
    job_links = extract_job_links(purple[1]) if len(purple) >= 2 else {}
    ...
    for item in pengiriman_list:
        ...
        if not ref or item.get("mediasi") != "-":
            continue
        if not _switch_to_ref(driver, ref):
            continue
