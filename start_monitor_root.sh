...
SHARED_SOCKET="/data/data/com.termux/files/home/.tmux-shared/telebot.sock"

# Cari socket tmux milik user biasa (untuk kontrol bot)
# Bisa override dengan export TMUX_SOCKET sebelum menjalankan skrip ini.
TMUX_SOCKET=${TMUX_SOCKET:-$($TMUX display-message -p '#{socket_path}' 2>/dev/null | head -n 1)}
[ -z "$TMUX_SOCKET" ] && TMUX_SOCKET=$(find /data/data/com.termux/files/usr -type s -name default 2>/dev/null | head -n 1)
[ -z "$TMUX_SOCKET" ] && TMUX_SOCKET="$SHARED_SOCKET"
